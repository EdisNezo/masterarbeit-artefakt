...
interface IdeState {
    ...
    // Chat State (for Survey System tracking)
    ...
    // Verbosity Mode
    verbosityMode: 'normal' | 'focus';
    toggleVerbosityMode: () => void;
...
`
    ...
    toggleVerbosityMode: () =>
        set((state) => ({ verbosityMode: state.verbosityMode === 'normal' ? 'focus' : 'normal' })),
    incrementChatInteraction: () =>
        set((state) => ({ chatInteractionCount: state.chatInteractionCount + 1 })),
